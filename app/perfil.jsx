...
export default function Perfil() {
  ...
  const { alumnos, talleres, clases, editarAlumno, eliminarAlumno, editarTaller, eliminarTaller, agregarClase, editarClase, eliminarClase, cambiarEstadoClase } = useAlumnos();
  ...
  const [editInstrumento, setEditInstrumento] = useState(INSTRUMENTOS.find(i => i.nombre === entidad?.instrumento) || INSTRUMENTOS[0]);
  ...
  const fechaFormateada = (d) => d.toLocaleDateString('es-CL');
  ...
  function handleAgregarClase() {
    ...
    setNuevaPlanificacion(''); setNuevasTareas('');
  ...
  function handleGuardarEdicion() {
    ...
    if (esTaller) {
      editarTaller({ ...entidad, nombre: editNombre, instrumento: editInstrumento.nombre, avatar: editInstrumento.emoji, diaSemana: editDia, hora: horaStr });
    } else {
  ...
  function handleGuardarClase() {
    editarClase(id, { ...claseEditando, planificacion: editPlanificacion, tareas: editTareas, estado: editEstado, fecha: fechaFormateada(editFechaClase) });
          ...
          <ScrollView contentContainerStyle={s.modalContenido} keyboardShouldPersistTaps="handled">
            ...
            <View style={s.instrumentosGrid}>
              {INSTRUMENTOS.map(inst => {
                const sel = editInstrumento?.nombre === inst.nombre;
                return (
                  <TouchableOpacity key={inst.nombre} style={[s.instrBtn, sel && s.instrBtnActivo]} onPress={() => setEditInstrumento(inst)} activeOpacity={0.7}>
