...
// Devuelve 'ok' si actualizó, 'gone' si el evento ya no existe en Calendar
// (404/410 — el usuario lo borró: hay que recrearlo, BUG-33), null si no aplica.
export async function editarEvento(accessToken, googleEventId, clase, entidadNombre) {
  ...
  try {
    ...
    if (!res.ok) {
      logWarn('gcal', 'PATCH de evento falló', { googleEventId, status: res.status });
      return null;
    }
    return 'ok';
  } catch (e) {
    if (e.message === 'TOKEN_EXPIRADO') throw e;
    logWarn('gcal', 'PATCH de evento falló', { googleEventId, error: e.message });
    return null;
  }
}
...
export async function eliminarEvento(accessToken, googleEventId) {
  ...
  try {
    ...
    logWarn('gcal', 'DELETE de evento falló', { googleEventId, status: res.status });
    return false;
  } catch (e) {
    if (e.message === 'TOKEN_EXPIRADO') throw e;
    logWarn('gcal', 'DELETE de evento falló', { googleEventId, error: e.message });
    return false;
  }
}
