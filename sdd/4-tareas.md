...
| BUG-33 | Un evento borrado en Calendar no se recrea (la app confía en el id guardado) | ✅ | Si el usuario borra el evento en Google, la app conserva `google_event_id` en la DB y nunca lo recrea: `sincronizarClasesExistentes` salta las clases que ya tienen id, y `editarEvento` hacía PATCH sobre un evento inexistente (404 ignorado). Fix: (1) `editarEvento` devuelve `'gone'` ante 404/410 y `editarClase` recrea el evento; (2) la sincronización usa la DB como única fuente de verdad del id (sin caer al id en memoria); (3) corrección de datos una vez: limpiar a `null` los `google_event_id` que apuntan a eventos ya borrados, para que la próxima sincronización los recree. |
