...
| BUG-35 | `conectarCalendar` no guarda el nuevo refresh token al reconectar (`duplicate key`) | ✅ | Reportado 2026-06-15 (log del dispositivo): `WARN [gcal] no se pudo guardar el refresh token {duplicate key value violates unique constraint "google_tokens_pkey"}`. Causa: `conectarCalendar` hacía `delete` + `insert` en `google_tokens`; al reconectar, la fila previa seguía y el `insert` chocaba con la PK (`user_id`), así que el **nuevo** refresh token no se persistía (seguía vivo el viejo — frágil). Las policies RLS son correctas (DELETE/INSERT/UPDATE con `auth.uid()=user_id`). Fix: reemplazar delete+insert por **`upsert`** idempotente (`onConflict: 'user_id'`, setea `updated_at`). |
