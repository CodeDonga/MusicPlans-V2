...
>
> **Bugs conocidos:** BUG-37..46 detectados en code-review (alto recall) el 2026-06-16 y corregidos en la misma tanda (ver tabla; pendiente verificación en dispositivo). BUG-20..28, ARQ-07..09 corregidos el 2026-06-11; BUG-29 corregido y verificado en dispositivo el 2026-06-12; BUG-30 (privilegios de DB) corregido el 2026-06-13; BUG-31 (refresco de Calendar roto por `invalid_client` — config de secrets, **causa raíz del "Calendar no funciona"**), BUG-32 (sync salta clases pasadas) y BUG-33 (eventos borrados no se recrean) corregidos y verificados contra Google el 2026-06-15.
...
| BUG-36 | Eliminar alumno/taller deja colgados en Calendar los eventos de sus clases pagadas | ✅ | Detectado 2026-06-15 al ver clases de alumnos eliminados en el calendario. `eliminarAlumno`/`eliminarTaller` solo borraban los eventos de las clases **no pagadas** (`filter(c => !c.pagada)`); las pagadas se conservan en la DB como historial financiero (`pagosHistoricos`) pero sus eventos de Google Calendar nunca se eliminaban → quedaban huérfanos. Fix: `conEventosDeDB(clases[id]||[], id)` (todas las clases) → `limpiarClasesBorradas` borra los eventos de todas; la fila de la clase pagada se conserva igual (lo maneja `borrarClasesNoPagadas` aparte). La sincronización ya ignora correctamente las clases sin entidad (no las recrea). |
| BUG-37 | Cobro de taller incorrecto al cambiar el roster/precio | ✅ | Detectado en code-review 2026-06-16. `perfil.jsx:montoCobro` dividía el total guardado al crear la clase (`valor_unitario` = `valor_por_alumno` × participantes de **ese** momento) por el número **actual** de participantes → cobro mal calculado si entraba/salía gente o cambiaba el precio (ej.: creada con 2 → `valor_unitario`=20000; ahora 3 → `round(20000/3)`=6667 en vez de 10000). Fix: la parte del participante es `valor_por_alumno` **actual**; si la clase tiene `valor_custom` (override del total), se reparte ese total entre los participantes actuales (`round(valor_custom / n)`). Spec `cobro-transferencia.md` §"cobro de taller" actualizada. |
| BUG-38 | Tras revocar acceso, reconectar Calendar no recupera el refresh token | ✅ | Detectado en code-review 2026-06-16 (regresión introducida por GC-08). `AuthContext:conectarCalendar` quitó `prompt:consent` de **todo** el flujo. Si el usuario revoca el acceso en Google (la Edge Function borra la fila), al reconectar, con el scope ya concedido y solo `access_type=offline`, Google **no reemite** `provider_refresh_token` → no se guarda → la sync muere ~55 min después (misma familia que BUG-30). Fix: forzar `prompt:'consent'` **solo** en el fallback OAuth (que se abre únicamente cuando no hay token utilizable); el reconnect normal sigue sin fricción porque el verify-first (`getCalendarAccessToken`) ni abre Google. Spec §3 actualizada. |
| BUG-39 | Cancelar una clase sin token deja el evento huérfano para siempre | ✅ | Detectado en code-review 2026-06-16. Cancelar con Calendar desconectado/token vencido salía en `reflejarClaseEnCalendar` (`if !token return`) sin borrar el evento ni limpiar el id; y `sincronizarClasesExistentes` **salteaba las canceladas** (`continue`), la única vía de recuperación → el evento quedaba en Calendar indefinidamente. Fix: la sync ahora enruta **todas** las clases por `aplicarEventoDeClase` (borra el evento de las canceladas con id) — ver BUG-44. |
| BUG-40 | Cancelar con DELETE fallido borra el id igual (huérfano irrastreable) | ✅ | Detectado en code-review 2026-06-16. `aplicarEventoDeClase` (branch `cancelada`) devolvía `null` (limpia el id) aunque `eliminarEvento` fallara (5xx transitorio) → el evento quedaba en Calendar sin id rastreable. Fix: si el DELETE no confirma éxito, se conserva `idActual` para reintentar en la próxima sync. |
| BUG-41 | Cada mutación de clase invoca la Edge Function aunque Calendar no esté conectado | ✅ | Detectado en code-review 2026-06-16. `reflejarClaseEnCalendar`/`limpiarClasesBorradas`/`eliminarClase`/`sincronizarClasesExistentes` llamaban `getCalendarAccessToken` sin chequear el flag de conexión: un profesor que **nunca** conectó Calendar disparaba, en cada crear/editar/cancelar, una invocación a `calendar-token` (404) + un `updateUser` (vía `desconectarCalendar`). Fix: gatear con `getCalendarToken()` (flag en `user_metadata`) antes de tocar Calendar. |
| BUG-42 | `cambiarEstadoClase` salta Calendar si la clase no está en memoria | ✅ | Detectado en code-review 2026-06-16. La reconciliación se condicionaba a `clase &&`; con el estado en memoria desfasado, cancelar actualizaba la DB pero no borraba el evento. Fix: el branch de cancelación enruta por `reflejarClaseEnCalendar` con `{ id, estado }` (el borrado solo necesita el id de la DB), independiente del objeto en memoria; la reactivación sigue usando la clase en memoria. |
| BUG-43 | El monto por clase diverge entre Finanzas y el mensaje de cobro | ✅ | Detectado en code-review 2026-06-16. `perfil.jsx` y `finanzas.jsx` calculaban el monto por clase con fórmulas distintas (perfil: `valor_custom ?? (valor_unitario || defecto)`; finanzas: `valor_custom ?? valor` sin el fallback histórico `valor_unitario`) → para la misma clase, Finanzas y el cobro podían mostrar montos distintos. Fix: helper compartido `lib/montos.js` (`valorPorDefecto`, `montoDeClase`) usado por ambas pantallas. Finanzas pasa a usar el `valor_unitario` histórico (más fiel al valor de la clase en su momento y consistente con el cobro). |
| BUG-44 | Lógica de sincronización de evento duplicada (sincronizar vs aplicarEventoDeClase) | ✅ | Detectado en code-review 2026-06-16 (deuda de altitud). `sincronizarClasesExistentes` reimplementaba el núcleo `editar→'gone'→crear` con semántica de error distinta (llamadas crudas + try/catch propio en vez de `safeGCal`) y salteaba canceladas. Fix: ahora enruta cada clase por `aplicarEventoDeClase(token, clase, entidad, idActual)` (ids leídos por lote de la DB), unificando creación/edición/recreación/**borrado de canceladas** y el manejo de token. Cierra el grueso de GC-09. |
| BUG-45 | Normalización de número de WhatsApp frágil | ✅ | Detectado en code-review 2026-06-16. `perfil.jsx:enviarCobro` solo anteponía `56` a números de 9 dígitos que empezaban con `9`; otros formatos guardados generaban links `wa.me` malformados sin aviso. Fix: normalización más robusta (quita no-dígitos; respeta el `56…` ya presente; antepone `56` si falta el código país; avisa si el número queda inválido). |
| BUG-46 | Formulario de datos de cobro en blanco en cold-start | ✅ | Detectado en code-review 2026-06-16. En `ajustes.jsx` los campos se sembraban solo en el inicializador de `useState`; en arranque en frío directo al tab Ajustes (la sesión aún resolviendo, el tab monta sin gate bloqueante) quedaban vacíos aunque hubiera datos guardados, y no se repoblaban al llegar la sesión. Fix: `useEffect` sobre `session?.user?.id` re-siembra los campos (nombre + datos de cobro) cuando la sesión resuelve, sin pisar ediciones posteriores. |
